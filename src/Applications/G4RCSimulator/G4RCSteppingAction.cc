// implementation of the G4RCSteppingAction class
...
G4RCSteppingAction::G4RCSteppingAction(G4RCEventAction* G4event, Event& theEvent)
    ...
    fEvent(theEvent)
{
	...
	if (fEvent.GetSimData().GetSimulationMode() == SimData::SimulationMode::eFast) {
		cout << "[INFO] G4RCSteppingAction::G4RCSteppingAction: Running Simulation in Fast mode." << endl;
		cout << "[INFO] G4RCSteppingAction::G4RCSteppingAction: Optical photons will be killed! " << endl;
...
void
G4RCSteppingAction::UserSteppingAction(const G4Step* step)
{
