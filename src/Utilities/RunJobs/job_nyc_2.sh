...
host=`/bin/hostname`
date=`/bin/date`
echo "Test JOB ">> Registro2cm.txt
echo "Machine: "$host>> Registro2cm.txt 
echo "Date: "$date>> Registro2cm.txt 
echo "Inicio del trabajo: $start_time">> Registro2cm.txt 

path="/opt/data"
inputfile="$path/CorsikaFlujos/1hora/S3_bga_003600_fluka.shw"
basefile="$path/Articulo/plomo/2cm/base"
jobsfile="$path/Articulo/plomo/2cm/jobs"
num_evts=416521
num_jobs=10
...
echo "Inputfile : $inputfile " >> Registro2cm.txt 
echo "srun -p GIRG -n 10 --qos=girg --pty /bin/bash" >> Registro2cm.txt 
echo "Job: meiga_Hodo__app  - Inicio: $start_time - Fin: $end_time" >> Registro2cm.txt 
echo "Fin del trabajo: $end_time" >> Registro2cm.txt 
