...
host=`/bin/hostname`
date=`/bin/date`
echo "Test JOB ">> Registro0cm.txt
echo "Machine: "$host>> Registro0cm.txt 
echo "Date: "$date>> Registro0cm.txt 
echo "Inicio del trabajo: $start_time">> Registro0cm.txt 

path="/opt/data"
inputfile="$path/CorsikaFlujos/1hora/S3_bga_003600_fluka.shw"
basefile="$path/Articulo/plomo/0/base"
jobsfile="$path/Articulo/plomo/0/jobs"
num_evts=416521
num_jobs=10
...
echo "Inputfile : $inputfile " >> Registro0cm.txt 
echo "srun -p GIRG -n 10 --qos=girg --pty /bin/bash" >> Registro0cm.txt 
echo "Job: meiga_Hodo__app  - Inicio: $start_time - Fin: $end_time" >> Registro0cm.txt 
echo "Fin del trabajo: $end_time" >> Registro0cm.txt 
